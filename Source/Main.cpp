...
int main(int argc, char** argv)
{
	...
	if (g_CommandLineOptions.NotifyImportanceStats)
	{
		size_t low = 0, mid = 0, high = 0;
		for (auto&& [handle, person] : peopleStorage.GetPeople())
		{
			switch (person.Importance)
			{
			case Person::LowImportance:  low++;  break;
			case Person::MidImportance:  mid++;  break;
			case Person::HighImportance: high++; break;
			}
		}
		std::cout << "Importance Stats:\n  Low  -> " << low << "\n  Mid  -> " << mid << "\n  High -> " << high << "\n TOTL -> " << (low + mid + high) << '\n';
	}
	...
	// }
	// Removed for cross-platform compatability.
	...
	StateHandling:
		...
		case ProgramState::PeopleSelection:
		{
			...
			{
				sf::Text authorityText(fontRegular, "    [Insert Author Name]\n[Insert Institution]", 24);
			...
			if (eMouseMove)
			{
				...
				for (auto&& [handle, shape] : r.GetPeopleBuffer())
				{
					if (shape.getGlobalBounds().contains((sf::Vector2f) eMouseMove->position))
					{
						const Person& p = peopleStorage.GetPeople().at(handle);
						switch (r.GetSortByOption())
						{
						case Renderer::SB_PRE_ISLAMIC:
						{
							if (p.Period != Person::HistoricalPeriod::PreIslamic)
							{
								goto SortMiss;
							}
							break;
						}
						case Renderer::SB_ISLAMIC:
						{
							if (p.Period != Person::HistoricalPeriod::Islamic)
							{
								goto SortMiss;
							}
							break;
						}
						case Renderer::SB_REPUBLIC:
						{
							if (p.Period != Person::HistoricalPeriod::Republic)
							{
								goto SortMiss;
							}
							break;
						}
						case Renderer::SB_ALL: break;
						}
