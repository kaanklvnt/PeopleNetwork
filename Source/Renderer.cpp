...
namespace PeopleNetwork
{
	...
	void Renderer::BuildPersonInfoBuffer(const Person& person)
	{
		...
		std::wstring bornYearOrUnknownText = person.Lifetime.first != 0 ? std::to_wstring(person.Lifetime.first) : L"bilinmiyor";
		std::wstring deadYearOrAliveText = person.Lifetime.second != 0 ? std::to_wstring(person.Lifetime.second) : L"günümüz";

		if (person.Lifetime.second == -1)
		{
			deadYearOrAliveText = L"bilinmiyor";
		}

		std::wstring contentNameHeader =
			person.Name +
			std::wstring(L" (") + bornYearOrUnknownText + L'-' + deadYearOrAliveText + L")\n";
		...
		wrapText(contentMain, m_FontData.Regular, *textContent, 24, (float) g_CommandLineOptions.WrapContentPixels);
	...
	void Renderer::InitSchoolInfo()
	{
		if (!m_TextureSymbolSchool.loadFromFile(g_CommandLineOptions.SymbolSchool))
		{
			LOG_ERROR("Failed to load school symbol image from file '" << g_CommandLineOptions.SymbolSchool << "'!");
			std::exit(EXIT_FAILURE);
		}

		m_SymbolSchool = sf::Sprite(m_TextureSymbolSchool);
		m_SymbolSchool.setPosition({ m_Window.getSize().x - 250.0f, 35.0f });
		m_SymbolSchool.setScale(m_SymbolSchool.getScale() / 4.5f);
		m_SymbolSchool.setColor({ 255, 255, 255, 200 });

		m_SchoolText = sf::Text(m_FontData.Bold, L"\"Güçlü Tarih,\n  Güçlü Okul\"");
		m_SchoolText.setPosition({ m_Window.getSize().x - 235.0f, 260.0f });
		m_SchoolText.setFillColor({ 0, 0, 0, 200 });
	}
	...
	void Renderer::RenderSchoolInfo()
	{
		m_Window.draw(m_SymbolSchool);
		m_Window.draw(m_SchoolText);
	}
	...
	void Renderer::RenderPersonTooltip(PRSHANDLE ih, std::wstring_view name, const sf::Font& font)
	{
		...
		sf::RectangleShape rectShape({ nameTextBounds.size.x + g_CommandLineOptions.XScaleFactor, nameTextBounds.size.y + g_CommandLineOptions.YScaleFactor });
	...
	{
		// FIXME: What the fuck is this piece of shit of code? I'd write this seriously if I were to give any fuck about the project.
		// As I do NOT give a fuck, I'll leave this rotten piece of shit as-is.

		sf::Sprite* preIslamic = (sf::Sprite*) m_SortByBuffer.at(SORT_BY_PRE_ISLAMIC).get();
		sf::Sprite* islamic = (sf::Sprite*) m_SortByBuffer.at(SORT_BY_ISLAMIC).get();
		sf::Sprite* republic = (sf::Sprite*) m_SortByBuffer.at(SORT_BY_REPUBLIC).get();

		sf::Color standard = { 0xff, 0xff, 0xff, 0xff };
		sf::Color modified = { 215, 215, 215, 0xff };
		switch (m_SortByOption)
		{
		case SB_ALL:
		{
			if (preIslamic)
				preIslamic->setColor(standard);
			islamic->setColor(standard);
			republic->setColor(standard);
